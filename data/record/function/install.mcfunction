...
tellraw @s [{"text": "<", "clickEvent": {"action": "run_command", "value": "/execute as @a run function record:setting"}, "hoverEvent": {"action": "show_text", "contents": "開啟設定"}}, {"text": "記錄位置", "color": "blue"}, "> 使用 ", {"text": "/function record:setting", "color": "aqua", "bold": true, "underlined": true}, " 開啟設定。"]
...
scoreboard objectives add rp_constant dummy
...
scoreboard players set #2 rp_constant 2