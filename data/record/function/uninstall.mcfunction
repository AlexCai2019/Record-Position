...
tellraw @s ["<", {"text": "記錄位置", "color": "blue"}, "> 若要重新安裝，請使用 ", {"text": "/function record:install", "underlined": true, "clickEvent": {"action": "run_command", "value": "/function record:install"}, "hoverEvent": {"action": "show_text", "contents": "執行 /function record:install"}}, " 指令。"]
...
scoreboard objectives remove rp_constant